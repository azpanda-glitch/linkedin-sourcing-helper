...
export function teamPostsUrl(ext) {
  if (!ext.team) return null;
  return linkedinPostsUrl(keywordString([ext.team, ext.company, "hiring"]));
}
...
export function programPeopleUrl(ext) {
  ...
  return linkedinPeopleUrl(keywordString([ext.program]));
}
...
export function buildQueries(ext) {
  ...
  return {
    ...
    hiringManager: [
      ...
      ext.team ? { label: `Posts: ${ext.team} + hiring`, url: teamPostsUrl(ext) } : { label: "", url: null },
      ...
      ext.keyPhrases?.[0]
        ? {
            ...
            url:
              ...
              linkedinPeopleUrl(keywordString([ext.keyPhrases[0], company]))
          }
        : { label: "", url: null },
