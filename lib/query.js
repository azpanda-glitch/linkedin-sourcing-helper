...
export function buildQueries(ext) {
  // Focused default: the exact role phrase (or title variants) scoped to the
  // company. ANDing a big skills group on top made the search so narrow /
  // noisy that it stopped returning the people you actually want.
  const role = cleanRoleName(roleName(ext));
  const coreBool = andJoin(
    [role ? quote(role) : orGroup(ext.titles), companyGroup(ext)],
    LINKEDIN
  );
  // Kept available as an explicit "broad" option rather than the default.
  ...
  return {
    ...
    sourcer: [
      ...
      { label: "Broad: titles + top skills", url: linkedinPeopleUrl(broadBool, ext.location) },
