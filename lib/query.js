...
// The primary query: exact role title AND company AND hiring.
// e.g. "Summer 2027 Intern - Marketing-Earned Media Specialist" AND "State Farm" AND hiring
export function roleHiringBoolean(ext) {
  const name = cleanRoleName(roleName(ext));
  if (!name) return "";
  return andJoin([quote(name), companyGroup(ext), "hiring"], LINKEDIN);
}
...
export function buildQueries(ext) {
  ...
  return {
    boolean: coreBool || titleCompanyBool,
    broadBoolean: broadBool,
    earlyCareer,
    hiringManager: [
      { label: "Hiring manager posts", url: roleHiringUrl(ext, { posts: true }) },
      { label: "Hiring manager posts (people)", url: roleHiringUrl(ext, { posts: false }) },
