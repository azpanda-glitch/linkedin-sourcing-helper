...
export function buildQueries(ext) {
  ...
  const role = generalizeRole(roleName(ext));
  const shown = coreBool || internHiringBoolean(ext) || titleCompanyBool;

  return {
    boolean: shown,
    broadBoolean: broadBool,
    // Surfaced so an over-budget query (which LinkedIn answers with zero
    // results and no explanation) is visible before it wastes the user's time.
    ...
    hiringManager: [
      ...
      earlyCareer
        ...
        : { label: "", url: null },
      { label: "People in this role at this company", url: rolePeopleUrl(ext), editable: true },
