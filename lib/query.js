...
// Internship postings have to say so. generalizeRole() strips "Intern" out of the
// title by design (nobody writes "Demand Intelligence Intern" in a post), which
// left `"demand intelligence" + "hiring" + "Atlassian"` pointing at the
// full-time req instead of the internship.
//
// One bare term, not an OR-group: with `+` every term is required, so
// `"intern" + "internship"` would demand both words appear.
function internTerm(earlyCareer, ...phrases) {
  if (!earlyCareer) return "";
  // Don't double up when the phrase already carries the word.
  if (/\bintern(ship)?s?\b|\bco-?ops?\b/i.test(phrases.filter(Boolean).join(" "))) return "";
  return INTERN_TERMS[0];
}
...
// Two different bars, because the two fields carry very different signal.
//
// The title is where early-career status is actually declared, so single words
// are trusted there.
const TITLE_EARLY_CAREER_RE =
...
// The description needs phrase-level evidence. Bare "university", "student" and
// "entry-level" are NOT enough: "Bachelor's degree from an accredited
// university" appears in nearly every full-time req, and matching it turned
// "Data Scientist 1" at PayPal into an intern search.
const DESC_EARLY_CAREER_RE =
  /\b(intern(ship)?s?\b|co-?op\s+(program|position|role|student)|(internship|intern|co-?op|new[\s-]?grad(uate)?|university|campus|student|apprenticeship|rotational)\s+program|currently\s+enrolled|pursuing\s+(a|an|your)?\s*(bachelor|master|mba|phd|doctora)|rising\s+(junior|senior|sophomore)|graduat(e|ing|ion)\s+(in|by|date|year)|expected\s+graduation|degree\s+in\s+progress|summer\s+20\d\d\s+(intern|cohort|program))/i;

export function isEarlyCareer(ext) {
  const titleHay = `${roleName(ext)} ${(ext.titles || []).join(" ")}`;
  if (TITLE_EARLY_CAREER_RE.test(titleHay)) return true;
  return DESC_EARLY_CAREER_RE.test(ext.description || "");
}
...
export function teamPostsKeywords(ext) {
  if (!ext.team) return "";
  return plusString([
    ext.team,
    internTerm(isEarlyCareer(ext), ext.team),
    "hiring",
    ext.company
  ]);
}
...
export function teamPostsUrl(ext) {
  const keywords = teamPostsKeywords(ext);
  return keywords ? linkedinPostsUrl(keywords) : null;
}

// Whoever already did this internship — the people worth asking how the process
// actually went. Only meaningful on an early-career posting.
export function pastInternsUrl(ext, head) {
  if (!isEarlyCareer(ext)) return null;
  const subject = head || ext.team;
  if (!subject) return null;
  return (
    companyPeopleTabUrl(ext, `${subject} intern`) ||
    linkedinPeopleUrl(plusString([subject, "intern", ext.company]))
  );
}
...
// Independent signals routinely collapse to the same search: the head noun and
// the team name often differ only in case ("demand intelligence" vs "Demand
// Intelligence"), and the top theme is frequently the head noun again. LinkedIn
// search is case-insensitive, so those render as two links that run one query.
// First occurrence wins, which keeps the editable primary link and its label.
...
export function buildQueries(ext) {
  ...
  // LinkedIn `+` syntax, head noun first, plus "intern" on early-career reqs:
  //   "analyst" + "intern" + "hiring" + "State Farm"
  const rolePostKeywords = head
    ? plusString([head, internTerm(earlyCareer, head), "hiring", company])
    : "";
  // The full phrase, kept as a narrower second option when it differs.
  const exactRoleKeywords = role
    ? plusString([role, internTerm(earlyCareer, role), "hiring", company])
    : "";
  ...
  // The People tab needs the company's slug, scraped from the posting's company
  // link. Without it those URLs are null and the global search bar is all we have.
  const hasCompanyTab = Boolean(ext.companySlug);
  ...
  return {
    ...
    hiringManager: dedupeLinks([
      // `editable` marks the links the keyword box drives.
      // Always the query in the keyword box, whatever it degraded to — role, or
      // "intern" + company, or company alone. Anything else and the box and the
      // link it drives disagree.
      shown
        ? { label: `Posts: ${shown}`, url: linkedinPostsUrl(shown), editable: true }
        : { label: "", url: null },
      ...
      // Skipped when it is already the primary query above.
      earlyCareer && internPostKeywords && internPostKeywords !== shown
      ...
      ext.team
        ? { label: `Posts: ${teamPostsKeywords(ext)}`, url: teamPostsUrl(ext) }
        : { label: "", url: null },
    ...
    sourcer: dedupeLinks([
      ...
      ext.program
        ...
        : { label: "", url: null },
      // People who already did this internship, to ask how the process went.
      earlyCareer && (head || ext.team)
        ? { label: `Past ${head || ext.team} interns`, url: pastInternsUrl(ext, head) }
        : { label: "", url: null },
      ...
      // through the global bar, where `+` applies. Suppressed when the People tab
      // is available: it runs the same query already scoped to the company by
      // URL, so the unscoped version is strictly worse, not an alternative.
      hasCompanyTab
        ? { label: "", url: null }
